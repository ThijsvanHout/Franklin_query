   ...
   /* $start=$_SESSION['user_page']*10;
    ...
    if (mysqli_num_rows($objecten) == 0) 
    {
        ...
        //zaterdag 13 aug 2016 toegevoegd
          ...
          echo "<h3 style=\"text-align:center; color:black;\">'".$_SESSION['kopText']."'</h3>";echo "hoeveel records zitten in de presentatie: ".$count_Limit_users."";
...
function functieQuery ()
{
  ...
  /*  if(!isSet($_SESSION['kopText']))
    ...
    $_SESSION['kopText'] = "Overzicht C# developers met voorkeur Regio Utrecht en ervaringlevel 8";
...
if(isSet($_SESSION['blad'])&& $_SESSION['blad'] !='user_blad2')    
{
  ...
  $_SESSION['user_page2']=0;// hierdoor kom ik in de eerste pagina van het kandidaat overzicht als ik uit de kandidaat.php kom na het bekijken van een profiel   
   ...
   /* $start=$_SESSION['user_page']*10;
    ...
    //SELECT DISTINCT achernaam, tussenvoegsel, voornaam, loginnaam, foto, cv WHERE functie_naam = “C# developer” AND regio_naam = “Utrecht”. 
     //$users = mysqli_query($connection, "SELECT DISTINCT `achternaam`, `tussenvoegsel`, `voornaam`, `user_inlognaam`, `foto`, `cv` FROM `kandiaten_vw`  WHERE  `functie_naam` = 'C# developer' AND  `regio_naam`  = 'Utrecht'  LIMIT $start,10") or die(mysqli_error());
     $users = mysqli_query($connection, "SELECT DISTINCT `achternaam`, `tussenvoegsel`, `voornaam`, `user_inlognaam`, `foto`, `cv` FROM `kandiaten_vw`  WHERE  `functie_naam` = 'C# developer' AND  `regio_naam`  = 'Utrecht'  LIMIT $start,10");
    ...
    $objecten = mysqli_query($connection, "SELECT * FROM user  WHERE `user_authorisatie`= 'usr' LIMIT $start,10") or die(mysqli_error());//weergave per 10
    $count_Limit_users = mysqli_num_rows($objecten);//tellen hoeveel users er (nog) in de ($objecten) presentatie  zitten
    ...
    if (mysqli_num_rows($objecten) == 0) 
    {
        ...
        echo "<th width=\"50\" align=\"left\">Loginnaam</th>";
        echo "<th width=\"50\" align=\"left\">Voornaam</th>";
        echo "<th width=\"50\" align=\"left\">Tussenvoegsel</th>"; 
        echo "<th width=\"50\" align=\"left\">Achternaam</th>";              
        //echo "<th width=\"50\" align=\"left\">Straat</th>";
